...
describe('DungeonConfig', () => {
  ...
  describe('Validation', () => {
    ...
    test('should report boss level without boss', () => {
      // Set as boss level without configuring a boss
      config.configureLevelPartial(5, {
        isBossLevel: true,
        bossOverride: null,  // Explicitly no boss
      });
      // Override the default boss (if any)
      config.levels.get(5).boss = null;
  ...
  describe('Summary', () => {
    test('should generate summary', () => {
      // Clear any default bosses first
      for (let i = 1; i <= 16; i++) {
        config.levels.get(i).boss = null;
        config.levels.get(i).bossOverride = null;
        config.levels.get(i).isBossLevel = false;
      }

      // Now set our test bosses
      config.setBoss(3, { type: 'SKELETON_KING' });
...
describe('CampaignPipeline', () => {
  ...
  describe('Pipeline Execution', () => {
    ...
    test('should emit events', () => {
      ...
      pipeline.on('test_event', (data) => {
        eventFired = true;
        eventData = data;
      });

      // Test the emit function directly
      pipeline.emit('test_event', { message: 'test' });
